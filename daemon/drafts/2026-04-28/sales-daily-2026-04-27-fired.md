...
### Pipeline state at EOD (canonical snapshot 03:50Z post-p019-transition)

| Stage | Count |
|-------|-------|
| pitched | 45 |
| prospect | 17 (was 13 at Apr 26 EOD; +p091/p092/p093 Apr 29 candidates ingested cycle 2034na, +p076 cold-dormant + p077 qmoney + p078 BlockRun + p004 StackingDAO backfilled cycle 2034ng) |
| lost | 5 |
| qualified | 2 |
| **lost-renewal-silent** | **2** (was 1, +p019 JingSwap silent-no-renewal at 03:09:43Z) |
| lost-reopen-conditional | 2 |
| closing | 2 |
| closed_pending_publish | 1 |
| closed | 0 (was 1, -p019 transitioned) |
...
| prospect-cold-dormant | 1 |
| prospect-pending-revision | 1 |
| support-case | 1 |
...
- `03:50Z` — **Day 10 EOD final pre-fire review** — pipeline section regenerated from canonical (87 → 91 post-backfill, p019 closed → lost-renewal-silent). Open commitments updated. Ready to fire.
...
- Apr 28 PT scout (p088/p089/p090) — locked + ingested + canonical-synced + 3 freshness re-checks passed (T-21h, T-12h51m, T-9h35m). Auto-fires 07:00Z Apr 28 (T+3h11m) via `scripts/fire-queue-2026-04-28.sh`.
...
- DT POST watch carries forward (T+38h+ silent at fire time, cross-confirmed silent on Arc IC #4 channel).
- JingSwap renewal: RESOLVED `lost-renewal-silent` at 03:10Z. Pattern flagged for separate analysis (3rd silent-no-renewal this week).
- p078 BlockRun.ai touch #2 ~Apr 30 (per Sales DRI cadence call to Arc on #570).
- #657 Publisher response watch carries forward (T+13h45m silent on factual correction at fire time).
